...
object User extends User with LongKeyedMetaMapper[User] {
  ...
  def isFullUser_?(u: User) = {
    ...
    if (u.firstName.is == null || u.firstName.length() <= 0) is = false;
    if (u.lastName.is == null || u.lastName.length() <= 0) is = false;
    if (u.birthday.is == null) is = false;
    println("")
    if (u.street.is == null || u.street.length() <= 0) is = false;
    if (u.postalCode.is == null || u.postalCode.length() != 5) is = false;
    if (u.city.is == null || u.city.length() <= 0) is = false;
    if (u.phoneNumber.is == null || u.phoneNumber.length() <= 0) is = false;
